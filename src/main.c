...
int main() {
  ...
  #ifndef WIN32
    log_trace("Windows sdl2 -m32 fix");
  ...
  #if LV_USE_LOG != 0
  log_trace("Ensuring lvgl logger exists");
  lv_log_register_print_cb(lv_log_print_g_cb);
  #endif

  /* Add a display
  * Use the 'monitor' driver which creates window on PC's monitor to simulate a display*/
  log_trace("Initializing lvgl window");
  lvDisplay    = lv_sdl_window_create(display_width, display_height);
  lvMouse      = lv_sdl_mouse_create();
  lvMouseWheel = lv_sdl_mousewheel_create();
  lvKeyboard   = lv_sdl_keyboard_create();

  log_trace("Allowing window resize");
  lv_sdl_window_set_resizeable(lvDisplay, true);
  ...
  log_trace("Setting up keyboard tracker");
  KEYS = (const bool *)SDL_GetKeyboardState(NULL);

  log_trace("Initializing app");
  if (app_setup()) {
    log_fatal("Error initializing app");
    return 1;
  }
